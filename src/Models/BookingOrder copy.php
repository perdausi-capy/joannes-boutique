...
class BookingOrder extends BaseModel
{
    ...
    public function create($data)
    {
        $sql = "INSERT INTO {$this->table} (
            user_id, order_type, item_id, event_date, event_end_date, rental_start, rental_end,
    ...
    public function checkAvailability($itemId, $orderType, $startDate, $endDate = null, $excludeOrderId = null)
    {
        ...
        if ($orderType === 'rental') {
            ...
            // For packages, check if event dates overlap (using event_date and event_end_date)
            $activeStatuses = ['pending', 'paid', 'verified'];
            $statusPlaceholders = implode(',', array_fill(0, count($activeStatuses), '?'));
            
            // Check for overlapping events using same logic as rentals
            // An event conflicts if: (startDate <= event_end_date) AND (endDate >= event_date)
            $sql = "SELECT order_id, event_date, event_end_date, payment_status, contact_name,
                           COALESCE(event_end_date, event_date) as effective_end_date
                    FROM {$this->table}
            ...
            // For single-day packages, startDate and endDate will be the same
            $params = array_merge([$itemId], $activeStatuses, [$startDate, $endDate ?? $startDate]);
            ...
            error_log("SQL: " . $sql);
            error_log("Parameters: item_id={$itemId}, start_date={$startDate}, end_date=" . ($endDate ?? $startDate));
        
            $stmt = $this->db->prepare($sql);
            $stmt->execute($params);
            $conflictingOrders = $stmt->fetchAll(\PDO::FETCH_ASSOC);
            
            error_log("Package availability check for item_id={$itemId}: Found " . count($conflictingOrders) . " conflicting orders");
        }
     ...
     */
/**
 * Check if a product/package is currently reserved
 * Returns array with 'is_reserved' (bool) and 'next_available_date' (string|null)
 */
public function getProductReservationStatus($itemId, $orderType = 'rental') {
    $currentDate = date('Y-m-d');
    
    if ($orderType === 'rental') {
        // Original rental logic (keep as is)
        ...
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$itemId, $currentDate]);
        ...
        // Package logic - check for events that haven't ended yet
        ...
        $sql = "SELECT 
                ...
                ORDER BY COALESCE(event_end_date, event_date) DESC
                LIMIT 1";
        ...
        if ($reservation) {
            // Use event_end_date if available, otherwise use event_date
            $endDate = !empty($reservation['event_end_date']) 
                ? new \DateTime($reservation['event_end_date']) 
                : new \DateTime($reservation['event_date']);
            
            $nextAvailable = clone $endDate;
            $nextAvailable->modify('+1 day');
            
            return [
                'is_reserved' => true,
                'next_available_date' => $nextAvailable->format('Y-m-d'),
