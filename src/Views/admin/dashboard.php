...
$stats = [
    ...
    'total_bookings'       => count($bookingOrderModel->findAll()),
];

// Compute overdue bookings (past event/rental dates)
$allBookings = $bookingOrderModel->findAll();
$now = new DateTime();
$overdueCount = 0;

foreach ($allBookings as $b) {
    $status = strtolower($b['payment_status'] ?? '');
    if (in_array($status, ['verified', 'completed', 'cancelled'])) continue;

    if ($b['order_type'] === 'rental' && !empty($b['rental_end'])) {
        $end = new DateTime($b['rental_end']);
        if ($end < $now) $overdueCount++;
    }

    if ($b['order_type'] === 'package' && !empty($b['event_date'])) {
        $event = new DateTime($b['event_date']);
        if ($event < $now) $overdueCount++;
    }
}

$stats['overdue_bookings'] = $overdueCount;

// Recent verified bookings
$recentBookings = $bookingOrderModel->findAll();
usort($recentBookings, fn($a, $b) => strtotime($b['created_at']) - strtotime($a['created_at']));
$recentBookings = array_slice($recentBookings, 0, 5); // show top 5 only
                    ...
                    <!-- <div class="stat-card fade-in bg-white p-6 rounded-2xl shadow-lg border border-gray-100" style="animation-delay: 0.4s;">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex-1">
                                ...
                                <p class="stat-number"><?= $stats['overdue_bookings']; ?></p>
                            </div>
                            <div class="icon-wrapper bg-red-100 text-red-700">
                                ...
                                <tr x-show="recentBookings.length === 0">
                                    <td colspan="7" class="px-8 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-400 text-sm mt-1">Recent bookings and rentals will appear here</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
            ...
            async refreshData() {
                    try {
                        ...
                        if (result.success && result.data && result.data.length > 0) {
                            ...
                            this.totalBookingsCount = result.count;
                        }
                    } catch (error) {
